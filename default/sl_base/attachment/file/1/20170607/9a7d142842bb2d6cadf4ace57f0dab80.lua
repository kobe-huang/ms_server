...
--                                      --
--                                      --
--170607  13:21:50    luoshui package 
--                                      --
--                                      --
--                                      --
------------------------------------------
...
local  function check_page_suoping( ... )
    mSleep(500)
    x, y = findMultiColorInRegionFuzzy({ 0x51BFDA, 1, 0, 0x51BFDA, 2, 0, 0x51BFDA, 3, 0, 0x51BFDA, 4, 0, 0x51BFDA, 11, 0, 0x51BFDC, 13, 0, 0x52C0DD, 22, 0, 0x53C3E0, 38, 0, 0x51C8E7, 48, 0, 0x4EC9EB, 278, -1, 0x37C9EB, 278, -10, 0x43CEEB, 297, -6, 0x46D0EB, 302, -6, 0x47CFEB, 302, -2, 0x43CEEB, 302, 10, 0x37CAEB, 319, 10, 0x37C8EB, 319, -4, 0x41CAEB, 319, -11, 0x45CAEB, 319, -16, 0x46C9EB, 312, -19, 0x4ACAEB, 307, -25, 0x54CCEB, 295, -25, 0x6AD5EB, 3, -1080, 0x3C4A7C, 13, -1080, 0x3C4A7C, 22, -1080, 0x3A4A7C, 33, -1080, 0x3A4A7C, 39, -1080, 0x3A4A7C, -1, -1080, 0x3C4A7C }, 90, 297, 27, 617, 1117);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("---锁屏界面-7.0---")
        return true
    else 
...
local function check_page_xiaoxi()
    x, y = findMultiColorInRegionFuzzy({ 0xFFFFFF, 37, 0, 0xFFFFFF, 48, 4, 0xFFFFFF, 53, 7, 0xFFFFFF, 53, 14, 0xCAEFFC, 59, 14, 0xCEF0FC, 29, 17, 0x12B7F5, 29, 9, 0xFFFFFF, -7, 12, 0xFFFFFF, -6, 25, 0xFFFFFF, 19, 32, 0xFFFFFF, 49, 32, 0xFFFFFF, 94, 32, 0xFFFFFF, 159, 34, 0x12B7F5, 171, 29, 0x12B7F5, 177, 25, 0x12B7F5, 182, 13, 0xFFFFFF, 177, 5, 0x12B7F5, 137, -1, 0x12B7F5, 137, 9, 0x12B7F5, 157, 16, 0xA0E2FB, 178, 14, 0x33C1F6, 180, 7, 0xFFFFFF, 149, 6, 0x12B7F5, 138, 6, 0x8BDCFA, 119, 6, 0x12B7F5, 110, 11, 0x12B7F5, 151, 28, 0x12B7F5, 210, 28, 0x12B7F5, 231, 28, 0x12B7F5, 234, 23, 0x12B7F5, 266, 11, 0x12B7F5, 271, 24, 0x12B7F5, 246, 15, 0x12B7F5, 242, 11, 0x12B7F5, 265, 12, 0x12B7F5, 238, 26, 0x12B7F5, 230, 13, 0x12B7F5, 239, 9, 0x12B7F5, 257, 11, 0x12B7F5 }, 90, 209, 71, 487, 106);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq消息界面1-7.0-8.0---");
        return true
    else
       x, y = findMultiColorInRegionFuzzy({ 0xFFFFFF, 43, 0, 0xFFFFFF, 45, 5, 0x87DAF9, 23, 15, 0xFFFFFF, 9, 8, 0xD2F1FD, 8, -1, 0xFFFFFF, -10, -11, 0xFFFFFF, -15, -6, 0xFFFFFF, -22, 10, 0xFFFFFF, -21, 27, 0xFFFFFF, 47, 28, 0xFFFFFF, 122, 6, 0xC8EEFC, 160, 7, 0x6ED1F6, 167, 11, 0x6ED1F6, 142, 24, 0x6ED1F6, 119, 4, 0xB2E7FA, 90, 9, 0x6ED1F6, 97, 25, 0x6ED1F6, 145, 31, 0x6ED1F6, 172, 30, 0x6ED1F6, 186, 27, 0x6ED1F6, 189, 16, 0x6ED1F6, 196, 17, 0x6ED1F6, 214, 25, 0x6ED1F6, 244, 18, 0x6ED1F6, 241, 4, 0x6ED1F6, 222, 4, 0x6ED1F6, 243, 24, 0x6ED1F6, 247, 15, 0x6ED1F6, 247, 13, 0x6ED1F6 }, 90, 213, 59, 482, 101);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
...
local function action_xiaoxi() --执行这个页面的操作--
    ...
    elseif gongneng == "group_message_add_friends" then
        ...
        mSleep(1500)
        --自己的群
        x, y = findMultiColorInRegionFuzzy({ 0xACACAC, 38, 4, 0xE9E9E9, 99, 4, 0xB9B9B9, 122, 8, 0xF0F0F0, 122, 11, 0xFFFFFF, 52, 9, 0xE8E8E8, 31, 5, 0xFDFDFD, 1, 4, 0xFFFFFF, -4, 5, 0x1C1C1C, 21, 11, 0x222222, 46, 15, 0x808080, 118, 15, 0x030303, 159, 8, 0xD8D8D8, 203, 4, 0xFFFFFF, 246, 3, 0x363636, 278, 6, 0xFFFFFF, 297, 11, 0xF0F0F0, 185, 15, 0xFFFFFF, 148, 8, 0xF3F3F3, 160, 8, 0x131313, 252, 9, 0xD6D6D6, 267, 12, 0xDBDBDB, 280, 15, 0xFFFFFF }, 90, 150, 143, 451, 158);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            cicky(60,80,50);   --点击返回
            mSleep(1500)
...
local function action_group_chat() --执行这个页面的操作--
    if gongneng == "group_add_friends" or gongneng == "copy_qq_number" then   --群加好友
        ...
        if group_num > 8 then
            for i=1,group_num - 8 do
                ...
                if z2 == z and n2 == n and m2 == m and o2 == o and p2 == p then
                    ...
                    local file = io.open("/var/touchelf/scripts/qq_group.txt","w+");  --初始化
                    file:write(1 .. "\r\n"); 
                    file:write(1 .. "\r\n");
                    file:close();
                    return false
            ...
            cicky(200,1085,50);   --点击一页的最后一个群
            mSleep(2000)
        else
            cicky(150,300+112*(group_num-1),50);          --点击第几个群
            mSleep(2000)
        end
        --群消息
        ...
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            ...
            cicky(150,300+112*group_num,50);          --点击下个群
            mSleep(2000)
            --群消息
            x, y = findMultiColorInRegionFuzzy({ 0xF0FAFE, 0, -5, 0xF0FAFE, 0, -17, 0xF0FAFE, -32, 7, 0xF0FAFE, -32, 12, 0xF0FAFE, -27, 14, 0xF0FAFE, -14, 3, 0xF0FAFE, 91, 5, 0xF0FAFE, 70, 3, 0xF0FAFE }, 90, 494, 69, 617, 100);
            if x ~= -1 and y ~= -1 then  -- 如果找到了
