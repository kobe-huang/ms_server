...
--                                             --
--                                             --
--170513  11:56:31    luoshui package 
--                                             --
--                                             --
--                                             --
-------------------------------------------------
...
----------------------begin:  class\app\class_app_phone_book.lua---------------------------------
--默认电话本用户参数---
--缩写pb： phone_book
...
default_pb_app_user = {
    ...
    user_data = {
        phone_add_contact_num = 0;       --添加电话联系人次数
        phone_add_contact_total_num = 0; --剩余电话联系人个数
    },    
    global_variable  = {
       add_phone_num = 0     --一次添加联系人次数
    }                    
}
...
phone_book_app_data = {
    ...
    app_ctrl_func_list = {
                    "default_func", --默认
                    "pb_add_contact",       --增加联系人
                    "pb_del_contact",             --删除联系人 
                },
    app_nv      = nil,
    app_nv_path = "",
    app_nv_name = "",
...
function phone_book_app_class:func_pb_add_contact( ... )
    local tmp_index = self.app_info.current_user_index;
    --当前添加电话通讯录总次数
    local num_phone_add = self.app_info.user_list[tmp_index].user_data.phone_add_contact_num
    if num_phone_add  < 200 then
        local add_num = math.random(200,250) - num_phone_add
        return add_num   --一次添加联系人次数
    else
        return 10      --一开始添加200-250个 后面每天加10*2个   
    end
end

function phone_book_app_class:func_pb_add_contact_finish(add_num)
    local tmp_index = self.app_info.current_user_index;
    local num_index1 = self.app_info.user_list[tmp_index].user_data.phone_add_contact_num  
    local num_index2 = self.app_info.user_list[tmp_index].user_data.phone_add_contact_total_num
    num_index1 = num_index1 + add_num
    num_index2 = num_index2 + add_num
    self.app_info.user_list[tmp_index].user_data.phone_add_contact_num = num_index1
    self.app_info.user_list[tmp_index].user_data.phone_add_contact_total_num = num_index2
end
...
function phone_book_app_class:func_pb_del_contact( ... )
    ...
    --添加电话通讯录次数
    local num_phone_add = self.app_info.user_list[tmp_index].user_data.phone_add_contact_num
    if num_phone_add  > 400 then  --联系人总数超过400 就删完所有联系人
        return  true
    else
        return  false   
    end
end
function phone_book_app_class:func_pb_del_contact_finish(del_num)
    local tmp_index = self.app_info.current_user_index;
    local del_index = self.app_info.user_list[tmp_index].user_data.phone_add_contact_total_num
    if del_num == 1 then
        self.app_info.user_list[tmp_index].user_data.phone_add_contact_total_num = del_index - 1 --每次减1
    elseif del_num == 0 then
        self.app_info.user_list[tmp_index].user_data.phone_add_contac_num = 0
        self.app_info.user_list[tmp_index].user_data.phone_add_contact_total_num = 0
    else
        wrning_info("删除联系人出现错误2")
        return false
    end
end
...
--     test_pb_app();
-- end
------------------end:  class\app\class_app_phone_book.lua-------------------------------------




----------------------begin:  class\app\class_app_weixin.lua---------------------------------
--默认微信用户参数---
default_wx_app_user = {
    user_name = "default_wx_account",  -- 用户名称
    user_snap_info = "1212121212112121212",            --用户特征信息
    user_data = {
        wx_add_contact_req_num = 0,                    --发起添加微信联系人总次数
        -- wx_add_contact_req_total_num= 0,               --    总次数
        wx_add_contact_success_num = 0,                 --  添加微信联系人成功次数
        -- wx_add_contact_success_total_num= 0,           --    总次数
        wx_add_contact_by_say_hello_num = 0,            -- 附近的人打招呼的次数
        -- wx_add_contact_by_say_hello_total_num= 0,      --   总次数
        wx_add_contact_by_group_num = 0,                -- 通过= 0,  --微信群加好友的次数
        -- wx_add_contact_by_group_total_num = 0,         --   总次数
        wx_add_contact_by_search_num = 0,              -- 搜索电话加好友
        wx_add_contact_by_search_phone_total_num = 0,  --   搜索电话加好友总次数
        wx_add_contact_by_phone_book_link_num = 0,     --  电话簿中，微信检测新的朋友，添加为好友
        ...
        wx_attention_public_number_num  = 0 ,               --关注公众号次数
        wx_attention_public_number_total_num  = 0 ,         --关注公众号总次数
    ...
    global_variable  = {
        add_fujin_num = 0 ,    --附近的人一次打招呼次数
        add_qun_num   = 0 ,   --微信群一次添加好友次数
        add_xinyou_num = 0   --新的朋友一次添加好友次数
    }              
}
--默认微信app数据--
weixin_app_data = {
    ...
    app_ctrl_func_list = {
                    ...
                    "wx_attention_public_number",              --关注公众号
...
--新的朋友
function weixin_app_class:func_wx_add_contact_by_phone_book_link(add_num)
    local tmp_index = self.app_info.current_user_index;
    ---新的朋友接受次数
    local num_accept_req = self.app_info.user_list[tmp_index].user_data.wx_accept_req_num
    --新的朋友一次添加次数
    local num_phone_book_add = math.random(4,6)
    if add_num == 1 then  --添加
        return num_phone_book_add
    elseif add_num == 2 then  --接受
        return num_accept_req
    else
        warning_info("获取新的朋友信息错误")
        return false
    end
end

function weixin_app_class:func_wx_add_contact_by_phone_book_link_finish(num_phone_book_add)
    local tmp_index = self.app_info.current_user_index;
    ---新的朋友添加次数
    local num_book_add = self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_phone_book_link_num
    --发起添加微信联系人总次数
    local num_contact_req = self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num
    ---新的朋友接受次数
    local num_accept_req = self.app_info.user_list[tmp_index].user_data.wx_accept_req_num
    ---添加微信联系人成功次数
    local num_wx_add = self.app_info.user_list[tmp_index].user_data.wx_add_contact_success_num
    if num_phone_book_add == 1 then     --添加
        self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_phone_book_link_num 
        = num_book_add + 1
        self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num = num_contact_req + 1 
    elseif num_phone_book_add == 2 then   --接受
        self.app_info.user_list[tmp_index].user_data.wx_accept_req_num = num_accept_req + 1
        self.app_info.user_list[tmp_index].user_data.wx_add_contact_success_num = num_wx_add + 1 
    else
        warning_info("打招呼统计错误")
        return false
    end
end

--附近的人打招呼
function weixin_app_class:func_wx_add_contact_by_say_hello( hello_num)
    --附近的人一次打招呼次数
    local num_hello = math.random(2,3)
    return num_hello
end
...
function weixin_app_class:func_wx_add_contact_by_say_hello_finish()
    local tmp_index = self.app_info.current_user_index;
    ---记录打招呼次数
    local num_hello = self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_say_hello_num
    ---记录发起添加微信联系人总次数
    local num_wx_add = self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num
    ---次数+1
    self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_say_hello_num = num_hello + 1
    self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num = num_wx_add + 1 
end

--微信群
function weixin_app_class:func_wx_add_contact_by_group()
    --微信群一次加好友次数
    local num_group_add = math.random(4,6)
    return num_group_add
end

function weixin_app_class:func_wx_add_contact_by_group_finish()
    local tmp_index = self.app_info.current_user_index;
    ---记录微信加好友次数
    local num_group = self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_group_num
    ---记录发起添加微信联系人总次数
    local num_req = self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num
    ---次数+1
    self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_group_num = num_group + 1
    self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num = num_req + 1 
end

--搜索号码加好友
function weixin_app_class:func_wx_add_contact_by_search_phone( ... )
    local tmp_index = self.app_info.current_user_index;
    ---记录微信搜索号码加好友次数
    local num_search = self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_search_num
    if num_search < 3 then
        return true
    else
        return false
    end
...
function weixin_app_class:func_wx_add_contact_by_search_phone_finish(add_num)
    local tmp_index = self.app_info.current_user_index;
    ---记录微信搜索号码加好友次数
    local num_search = self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_search_num
      ---记录发起添加微信联系人总次数
    local num_req = self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num
    if add_num == 1 then
        if num_search > math.random(10,15) then
            --初始化
            self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_search_num  = 0
        else
            --次数+1
            self.app_info.user_list[tmp_index].user_data.wx_add_contact_by_search_num = num_search + 1
        end
    end
    self.app_info.user_list[tmp_index].user_data.wx_add_contact_req_num = num_req + 1 
end
...
--关注公众号
function weixin_app_class:func_wx_attention_public_number( ... )
    local tmp_index = self.app_info.current_user_index;
    --关注公众号次数
    local num_search = self.app_info.user_list[tmp_index].user_data.wx_attention_public_number_num
    if num_search < 2 then
        return true
    else
        return false
    end
...
function weixin_app_class:func_wx_attention_public_number_finish(add_num)
    local tmp_index = self.app_info.current_user_index;
    --关注公众号次数
    local num_attention= self.app_info.user_list[tmp_index].user_data.wx_attention_public_number_num
    --关注公众号总次数
    local num_attention_total= self.app_info.user_list[tmp_index].user_data.wx_attention_public_number_total_num
    if add_num == 1 then
        if attention > 10  then 
            self.app_info.user_list[tmp_index].user_data.func_wx_attention_public_number = 0
        else
            self.app_info.user_list[tmp_index].user_data.func_wx_attention_public_number=num_attention + 1
        end
    end
    self.app_info.user_list[tmp_index].user_data.wx_attention_public_number_total_num= num_attention_total + 1 
end
-------------------------------------------test 部分-------------------------
...
--     test_wx_app();
-- end
------------------end:  class\app\class_app_weixin.lua-------------------------------------
...
local function action_phone_book_opt() --执行这个页面的操作--
    -- if nv_read_nv_item("photo") ~= nil and gongneng == 5.2  then
    ...
    --     mSleep(1000)
    -- end
    -- if nv_read_nv_item("photo") ~= nil and gongneng == 5.3  then
    --     if nv_read_nv_item("time") < 14 then
    ...
    --         log_info("删除所有联系人")
    --         mSleep(1000)
    --         notifyMessage("删除所有联系人");
    --         mSleep(1000)
    --     end
    -- elseif nv_read_nv_item("photo") == nil and gongneng == 5.3 then
    --     warning_info("没到时间，不删除联系人");
    --     mSleep(1000)
    --     return true
    -- else
...
local function action_faxian()     --执行这个页面的操作--
    if gongneng == 3.1 or gongneng == 3.4 then   --打招呼   站街扫街
        ...
        shanghua();  --向上滑动
        mSleep(1000); 
        if gps_x[1] ~= nil or gps_y[1] ~= nil then
    ...
    elseif gongneng == 3.3 or gongneng == 2.41 or gongneng == 3.5 or gongneng == 3.6 or gongneng == 3.7then    --朋友圈
        shanghua();  --向上滑动
        cicky( 297,209,50);     
        mSleep(3000);
        ---是不是第一次打开
...
local function action_suoyoulianxiren_delete()     --执行这个页面的操作--
    ...
    --     if nv_read_nv_item("time") < 16  then
    --         warning_info("没到时间，不删除联系人");
    --         mSleep(1000)
    --         return true
    --     end
    ...
    -- end
    local my_pb_app = {}
    local my_pb_app = phone_book_app_class:new();
    local num_book_del =  my_pb_app:do_func("pb_del_contact")   --删除联系人
    if num_book_del == true then
        log_info("删除全部联系人")
    else
        warning_info("联系人没到指定个数，不删除")
        return false
    end
    my_pb_app:finish_func("pb_del_contact",1)     --联系人总个数 - 1
    mSleep(1000)
...
local my_pb_app = {}
local my_pb_app = phone_book_app_class:new();
local num_book_add =  my_pb_app:do_func("pb_add_contact")   --添加联系人个数
local num_book_add2 = num_book_add

local function action_suoyoulianxiren_add()     --执行这个页面的操作--
    mSleep(1000)
    notifyMessage( num_book_add) ;    --增加联系人次数
    mSleep(1000)
    notifyMessage( num_book_add2) ;    --增加联系人次数
    mSleep(1000)
    -- --读取nv（本地文件） 看添加过联系人没
    -- if nv_read_nv_item("photo") ~= nil then
    --     if nv_read_nv_item("photo") == 1  then
    ...
    --         nv_write_nv_item("time", time)   --更新天数 这是2倍的天数 一天会运行2次 
    --         warning_info("联系人已加,不用在加");
    --         my_pb_app:finish_func("pb_add_contact",200)   --添加联系人次数记录在app_nv中
    --         mSleep(1000)
    --         return true
    --     else
    --         mSleep(1000)
    ...
    if num_book_add == 0 then
        ...
        warning_info("联系人已加完");
        mSleep(1000)
        if num_book_add2 == 10 then
            return true
        else
            return reset_ms_server()  --加完复位
        end
    end
    my_pb_app:finish_func("pb_add_contact",1)   --添加联系人次数记录在app_nv中  每加一次1
...
function suoyoulianxiren_page:action()     --执行这个页面的操作--
    if gongneng == 5.3 then  --删除
        if true == is_exsist_contact() then
            ...
            mSleep(1000)
            my_pb_app:finish_func("pb_del_contact",0)   --初始化联系人个数为0
...
local function action_sousuojiahaoyou()     --操作
    local my_app = {}
    local my_wx_app = weixin_app_class:new();
    if gongneng == 2.11 then
        my_name, my_number = generate_contact_info(); --产生随机的信息
        mSleep(500);
        --设置电话号码
        if gong_neng == "新号" then   --加新号为好友
            ...
            xinhao_info =xinhao_wxh[math.random(#xinhao_wxh)]  --随机获取一个新养的号的微信号  
            nv_write_nv_item("xinhao_info",xinhao_info)      --数据写入nv
            mSleep(500)
            inputText(xinhao_info);       --输入微信号
            mSleep(500)
            my_wx_app:finish_func("wx_add_contact_by_search_phone",2)      --记录微信添加好友次数   
        elseif gong_neng == "指定" then   --加指定微信号
            ---获取搜索次数
            local num_sousuo = my_wx_app:do_func("wx_add_contact_by_search_phone")
            if num_sousuo == true then  --小于3
            ...
            --记录搜索次数   5天以后初始化一次
            my_wx_app:finish_func("wx_add_contact_by_search_phone",1)   
...
local function action_jiagongzhonghao()     --操作
    if gongneng == 2.12 then
        --判断需不需要关注指定公众号
        if gongzhonghao[1] ~= nil then
            local my_app = {}
            local my_wx_app = weixin_app_class:new();
            --获取关注次数
            local num_guanzhu = my_wx_app:do_func("wx_attention_public_number")
            if num_guanzhu == true then  --没关注指定的公众号
                inputText(gongzhonghao[math.random(#gongzhonghao)]);     --输入指定公众号
                --记录关注公众号次数  5天初始化一次
                my_wx_app:finish_func("wx_attention_public_number",1)
            else
                inputText(gongzhonghao2[math.random(#gongzhonghao2)]);     --输入随机一个
                --记录关注公众号总次数
                my_wx_app:finish_func("wx_attention_public_number",2)
            end
            -- --判断关注指定公众号没
...
local my_wx_app = {}
local my_wx_app = weixin_app_class:new();
local num_add_xin = my_wx_app:do_func("wx_add_contact_by_phone_book_link",1)  --添加次数
local num_add_accept = my_wx_app:do_func("wx_add_contact_by_phone_book_link",2)  --接受总次数

local function action_xindepengyou()     --操作
    local my_wx_app = {}
    local my_wx_app = weixin_app_class:new();
    if  gongneng == 2.2  then 
        if strategy[1] == "3" then  --养号
            if num_add_accept > 100 then
        ...
        for i=1,10 do
            ...
            if page_array["page_buyanzheng"]:quick_check_page() == true  then ---不需要验证
                my_wx_app:finish_func("wx_add_contact_by_phone_book_link",2)  --接受次数记录
                page_array["page_buyanzheng"]:enter();
                break
            end
            if page_array["page_yaoyanzheng"]:quick_check_page() == true  then --要验证
               my_wx_app:finish_func("wx_add_contact_by_phone_book_link",1)  --添加次数记录
...
local function action_yaoyanzheng()     --操作
    if gongneng == 2.2 then
        cicky( 579,253,50);   --点击叉叉
        mSleep(1500);
        inputText(text_yanzheng[math.random(#text_yanzheng)]);     --输入消息
        mSleep(1500);
...
local function action_qunliao()     --操作
    if gongneng == 2.3  then   --微信群加好友
        ...
        local my_wx_app = {}
        local my_wx_app = weixin_app_class:new();
        local num_qunjiaren = my_wx_app:do_func("wx_add_contact_by_group")   --获取添加次数
        log_info(num_qunjiaren)
        notifyMessage(num_qunjiaren)
        mSleep(1000)
    ...
    if gongneng == 2.32  then   --微信群加好友
        ...
        x, y = findMultiColorInRegionFuzzy({ 0x1AAD19, 44, 4, 0x78CF78, 98, 5, 0xA6DFA6, 159, 3, 0x33B632, 191, 3, 0xF1FAF1, 204, 8, 0x24B123, 206, 26, 0x1AAD19, 206, 28, 0xFFFFFF, 186, 23, 0x30B52F, 169, 14, 0x1AAD19, 154, 16, 0xF2FAF2, 148, 23, 0x83D383, 114, 23, 0x1AAD19, 107, 11, 0x1AAD19, 118, 14, 0x5CC55C, 74, 22, 0x52C151, 79, 14, 0x63C762, 83, 19, 0x1AAD19, 50, 29, 0x68C967, 40, 16, 0x1AAD19, 61, 21, 0x1AAD19, 39, 33, 0x1AAD19, -5, 19, 0x1AAD19, 7, 12, 0x1AAD19, 18, 29, 0x1AAD19, 3, 27, 0x22B021 }, 90, 211, 830, 422, 863);
         if  x ~= -1 and y ~= -1  then  -- 如果找到了
            ...
            mSleep(50);
            touchUp(0)
            my_wx_app:func_finish("wx_add_contact_by_group")   --记录请求添加次数
            mSleep(5000)
        end
        --添加到通讯录2
...
local function action_sousuo()     --执行这个页面的操作--
    ...
    elseif gongneng == 1.5 then  --跟新号发消息
        if nv_read_nv_item("xinhao_info") == nil then  --读取新号的信息
            ...
            local a = nv_read_nv_item("xinhao_info")
            inputText(a)
        end
        mSleep(5000); 
        for i=1,5 do
...
local function action_liaotian()     --执行这个页面的操作--
    ...
    elseif gongneng == 1.4 then   ---回复一下  --养号
        ...
        for i=1,a do
            local aa = math.random(2500,10000)
...
local fj_time = 1  --错误次数
local my_wx_app ={} 
local my_wx_app = weixin_app_class:new();   --初始化微信
local num_hello = my_wx_app:do_func("wx_add_contact_by_say_hello")
local function action_fujinderen()     --操作
    ...
    if gongneng == 3.1 then
        ...
        local my_wx_app ={} 
        local my_wx_app = weixin_app_class:new();   --初始化微信app_server
        --打招呼次数每次加1  发起添加好友总次数加1
        local num_hello = my_wx_app:finish_func("wx_add_contact_by_say_hello")   
...
local function action_dazhaohu()     --操作
    if gongneng == 3.1  then
        ...
        if x>0 and y>0 then   --打招呼
            ...
            mSleep(2000);
            m = math.random(21,467)
            o = math.random(130,950)
            p = math.random(130,950)
            if o > p then
                o2 = o 
                p2 = p 
            else
        ...
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            ...
            if gongneng == 3.1 then
                for i=1,m do
                    ...
                    mSleep(math.random(100,2000))
                    move(n,o2, n,p2,5) --随机滑动
                end
            end
