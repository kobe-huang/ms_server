...
--                                      --
--                                      --
--170525  15:18:59    luoshui package 
--                                      --
--                                      --
--                                      --
------------------------------------------
...
function qq_warning_info(out_info)  ---错误处理函数   
    ...
    if nil ~= func_error_time then
        ...
        elseif error_time2 == 3 then
            warning_info("警告3次,继续下一个");
            if appRunning("com.tencent.mqq") then 
                appKill("com.tencent.mqq");
                mSleep(1500);
            end
...
local function check_page_xiaoxi()
    x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, 75, 7, 0xFFFFFF, 120, 11, 0xFFFFFF, 190, 14, 0x12B7F5, 255, 9, 0x12B7F5, 274, 13, 0x12B7F5, 286, 21, 0x12B7F5, 235, 37, 0x12B7F5, 155, 33, 0xFFFFFF, 76, 28, 0xFFFFFF, 40, 32, 0x12B7F5, 10, 39, 0x12B7F5, 147, 65, 0x12B7F5, 244, 59, 0x12B7F5, 351, 34, 0x12B7F5, 409, 23, 0x12B7F5, 459, 19, 0x12B7F5, 468, 36, 0x12B7F5, 452, 36, 0x32C1F6, -52, 1013, 0x12B7F5, -40, 1032, 0x12B7F5, -50, 1040, 0x12B7F5, -51, 1071, 0x14ADE7, -35, 1070, 0x14ADE7, -44, 1061, 0xFFFFFF, -48, 1063, 0xAEE3F6 }, 90, 93, 50, 613, 1121);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq消息界面1-7.0---");
        return true
    else
        x, y = findMultiColorInRegionFuzzy({ 0xFFFFFF, 59, 2, 0xFFFFFF, 16, 17, 0x12B7F5, 17, 6, 0x38C2F6, 47, 4, 0xFFFFFF, -3, 2, 0xFFFFFF, 3, -2, 0xFFFFFF, -80, 11, 0x6ED1F6, -65, 30, 0x6ED1F6, -90, 5, 0x6ED1F6, -28, 28, 0xFFFFFF, 98, 4, 0x6ED1F6, 160, 28, 0x6ED1F6, 198, 2, 0x6ED1F6, 133, -3, 0x6ED1F6, 144, 16, 0x6ED1F6, 130, 6, 0x6ED1F6, 148, 13, 0xFFFFFF, 155, 17, 0xFCFEFF, 140, 31, 0x6ED1F6, 179, 44, 0x6ED1F6, 226, 30, 0x6ED1F6, 333, 1, 0x6ED1F6, 346, 15, 0xFFFFFF, 364, 29, 0x6ED1F6, 364, -6, 0x6ED1F6, 340, 5, 0x6ED1F6, 361, 22, 0x6ED1F6, 306, 5, 0x6ED1F6, 304, 4, 0x6ED1F6, 303, 3, 0x6ED1F6 }, 90, 147, 61, 601, 111);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
...
local function action_xiaoxi() --执行这个页面的操作--
    if gongneng == "add_group" or gongneng == "add_friends" or 
    gongneng == "pull_people_into_group" then
    ...
    elseif gongneng == "group_message_add_friends" then
        ...
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            qq_warning_info("没有消息了")
            return false
        end
        cicky( 177,265,50);   --点击第一条消息
        mSleep(1500)
        ...
        elseif page_array["page_tencent_news"]:quick_check_page() == true then
            ...
            cicky(60,80,50);   --点击返回
            mSleep(1500)
            move( 504,262, 100,262,8);   --滑动删除
            mSleep(1000)
            cicky( 603,262,50);   --点击删除
            mSleep(1500)
            page_array["page_xiaoxi"]:enter()
        end
    elseif gongneng == "del_message" then
        move( 504,262, 100,262,8);   --滑动删除
        mSleep(1000)
        cicky( 603,262,50);   --点击删除
        mSleep(1500)
...
local function check_page_lianxiren()
    x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, 49, -3, 0x12B7F5, 76, 2, 0x12B7F5, 111, 6, 0x12B7F5, 138, 6, 0x12B7F5, 166, 6, 0x12B7F5, 226, 6, 0x12B7F5, 310, 6, 0x12B7F5, 401, 6, 0x12B7F5, 128, 18, 0xABE6FB, 187, 18, 0x12B7F5, 215, 20, 0x12B7F5, 121, 32, 0x12B7F5, 194, 30, 0x12B7F5, 207, 30, 0x12B7F5, 441, 7, 0x12B7F5, 462, 9, 0x12B7F5, 473, 20, 0x3CC4F7, 442, 33, 0x12B7F5, 426, 21, 0xF9FDFF, 480, 16, 0x12B7F5, 480, 16, 0x12B7F5, 476, 14, 0x12B7F5 }, 90, 148, 58, 628, 94);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq联系人界面1-7.0---");
        return true
    else
...
local function action_lianxiren() --执行这个页面的操作--
    ...
    elseif gongneng == "group_add_friends" or gongneng == "pull_people_into_group" then
        cicky( 310,279,50); --点击
        mSleep(1000)
...
local function check_page_dongtai()
    x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, 27, 5, 0x12B7F5, 31, 26, 0x12B7F5, -5, 21, 0x12B7F5, -11, 9, 0x12B7F5, 18, 6, 0x2FC0F6, 14, 20, 0x12B7F5, -296, 17, 0x12B7F5, -247, 28, 0x18B9F5, -239, 3, 0x12B7F5, -289, 24, 0xBBEAFC, -216, 25, 0x12B7F5, -47, 26, 0x12B7F5, -149, 16, 0x12B7F5, -89, -9, 0x12B7F5, -287, 23, 0x12B7F5, -343, 48, 0x12B7F5, -328, -4, 0x12B7F5, -376, -5, 0x12B7F5, -355, 25, 0x12B7F5, -327, -17, 0x12B7F5, -372, -18, 0x12B7F5, -418, -16, 0x12B7F5, -389, 2, 0x12B7F5 }, 90, 163, 49, 612, 115);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq动态界面----");
        return true
    else
...
local function action_dongtai() --执行这个页面的操作--
    ...
    elseif gongneng == "add_friends" or gongneng == "add_group"or gongneng == "group_add_friends" or gongneng == "pull_people_into_group" then
        cicky( 321,1067,50);  --点击联系人
        mSleep(1000)
        page_array["page_lianxiren"]:enter()
    elseif gongneng == "group_message_add_friends" then
...
local function check_page_tongxunlu()
    x, y = findMultiColorInRegionFuzzy({ 0xFFFFFF, 87, 2, 0x12B7F5, 73, 27, 0x12B7F5, 29, 27, 0x12B7F5, -17, 15, 0x12B7F5, 45, 6, 0x12B7F5, 74, 25, 0x12B7F5, 78, 26, 0x89DBFA, 286, 5, 0x9EE2FB, 329, 32, 0x12B7F5, 336, -2, 0x12B7F5, 302, -3, 0x12B7F5, 292, 18, 0x12B7F5, 330, 14, 0x12B7F5, 268, 2, 0x12B7F5, -249, 3, 0x12B7F5, -169, 30, 0x12B7F5, -250, -29, 0x12B7F5, -194, 11, 0x12B7F5, -69, 11, 0x12B7F5, -8, 8, 0x12B7F5, 62, 2, 0x12B7F5, 70, 13, 0x12B7F5, -4, 10, 0x84DAFA, 109, -5, 0x12B7F5, 190, 14, 0x12B7F5, 244, 5, 0x12B7F5, 213, -7, 0x12B7F5 }, 90, 37, 42, 623, 103);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("---qq通讯录界面-7.0---")
        return 1
    else
        x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, 48, 14, 0x12B7F5, 15, -13, 0xB3E8FC, 54, -8, 0x12B7F5, 218, -15, 0x12B7F5, 312, -6, 0xFFFFFF, 393, -6, 0x12B7F5, 392, 9, 0x12B7F5, 259, 19, 0x12B7F5, 207, 0, 0x97DFFB, 208, -3, 0xFFFFFF, 325, -3, 0x12B7F5, 372, -1, 0xFFFFFF, 509, -1, 0x12B7F5, 569, -5, 0x12B7F5, 558, -14, 0x12B7F5, 588, 6, 0x12B7F5, 588, 6, 0x12B7F5, 546, -4, 0x12B7F5 }, 90, 33, 68, 621, 102);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            log_info("---qq绑定手机号界面---")
            return 2
        else
...
local function action_tongxunlu() --执行这个页面的操作--
    cicky( 591,84,50)  --点击关闭
    mSleep(1000)
    page_array["page_xiaoxi"]:enter()
end

local function action_binding_photo_number()
    cicky(  43,82,50);   --点击关闭
    mSleep(1000)
    page_array["page_xiaoxi"]:enter()
end
...
function tongxunlu_page:enter()        --进入页面后的动作--
    if 1 == self.check_page(self) then
        return self.action(self)
    elseif 2 == self.check_page(self) then
        return action_binding_photo_number()
    else
        self:error_handling();
    end
end
...
function tongxunlu_page:check_page()  --检查是否是在当前页面--
    ...
    while 3 > time do
        mSleep(1000*time);   --休眠一会会
        if 1 ==  check_page_tongxunlu() then 
            return 1;
        elseif 2 == check_page_binding_photo_number() then
            return 2
        else          
            time = time + 1;
        end
...
local function check_page_add_contacts()
    x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, -14, 22, 0xE7F8FE, 25, 42, 0x12B7F5, 65, 43, 0x12B7F5, 115, 35, 0x12B7F5, 122, 13, 0x12B7F5, 34, 5, 0x5CCEF8, 0, 10, 0xE1F6FE, 71, 33, 0x12B7F5, 100, 22, 0x12B7F5, 18, 11, 0x12B7F5, -19, 18, 0x12B7F5, 280, 8, 0xFFFFFF, 324, 14, 0x12B7F5, 279, 39, 0x12B7F5, 248, 15, 0x12B7F5, 318, 25, 0x8DDCFA, 287, 23, 0x12B7F5, 540, 7, 0x12B7F5, 581, 23, 0x12B7F5, 527, 17, 0x12B7F5, 505, 21, 0x12B7F5, 83, 94, 0xFFFFFF, 93, 108, 0xFFFFFF, 42, 105, 0xFFFFFF, 86, 98, 0xFDFEFE, 43, 119, 0xFFFFFF, 270, 119, 0xDCDCDC, 297, 118, 0xC8C8C8, 264, 114, 0x999999, 301, 120, 0xFFFFFF, 325, 121, 0xFFFFFF, 270, 92, 0xFFFFFF }, 90, 14, 63, 614, 184);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("---搜索添加1界面-7.0---");
        return true
    else
...
local function action_input_search() --执行这个页面的操作--
    ...
    elseif gongneng == "add_group" then  --添加群
        if text_group_info == nil then
            qq_warning_info("没有添加群信息，搜索不了")
            return false
        end
        inputText(text_group_info[math.random(#text_group_info)])       --输入群信息搜索
        mSleep(1000)
        cicky( 143,269,50);   --点击找群
        mSleep(2000)
        if page_array["page_sq_result"]:check_page() == true then
            page_array["page_sq_result"]:action()
        elseif page_array["page_application_jiaqun"]:check_page() == true then
            page_array["page_application_jiaqun"]:action()
        else
...
local function check_page_shy_add_verification()
    ...
    if x ~= -1 and y ~= -1 then  -- 如果找到了
       ...
       x, y = findMultiColorInRegionFuzzy({ 0x34C0F5, 37, 21, 0xFFFFFF, 95, -9, 0x34C0F5, 31, -16, 0x34C0F5, 70, 4, 0x9FE1FA, 84, 4, 0x34C0F5, 50, 13, 0xEDFAFE, 246, 8, 0x78D5F8, 301, 13, 0x34C0F5, 325, 2, 0xBBEAFC, 345, 0, 0xB6E8FB, 347, 10, 0xFFFFFF, 281, 13, 0xFFFFFF, 261, -2, 0x34C0F5, 328, -3, 0xD0F0FD, 508, -6, 0x34C0F5, 570, 28, 0x34C0F5, 591, 16, 0xFFFFFF, 594, 5, 0xFFFFFF, 563, -13, 0x34C0F5, 525, 0, 0x34C0F5, 551, 3, 0x34C0F5, 573, 3, 0x34C0F5 }, 90, 22, 59, 616, 103);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            log_info("---- 搜索加好友验证界面2-7.0---");
            return true
        else
            return false
        end
...
local function action_shy_add_verification() --执行这个页面的操作--
    if gongneng == "add_friends" then  --加好友
        if text_shy_verification ~= nil then
...
local function check_page_shy_send_verification()
    x, y = findMultiColorInRegionFuzzy({ 0xF0FAFE, -4, 25, 0xF0FAFE, 55, 23, 0xFFFFFF, 85, 7, 0xFCFEFF, 32, -7, 0x6ED1F6, 132, 0, 0x91DCF8, 103, 24, 0xF4FBFE, 4, 7, 0x6ED1F6, 37, 0, 0x6ED1F6, 121, 12, 0x6ED1F6, 264, 28, 0xFFFFFF, 347, 23, 0x6ED1F6, 339, 4, 0xB9E9FB, 249, 0, 0x6ED1F6, 294, 22, 0xFFFFFF, 317, 22, 0x6ED1F6, 568, 6, 0x6ED1F6, 585, 22, 0x6ED1F6, 530, 12, 0x6ED1F6, 561, 0, 0xE4F6FD, 561, 0, 0xE4F6FD }, 90, 31, 62, 620, 97);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("---- 加好友发送验证界面1-7.0----");
        return true
    else
        x, y = findMultiColorInRegionFuzzy({ 0xE7F8FE, -11, 10, 0xE7F8FE, 6, 38, 0x12B7F5, 39, -1, 0x12B7F5, 88, -5, 0x12B7F5, 132, -4, 0x12B7F5, 139, 25, 0xCBEFFD, 110, 28, 0x12B7F5, 49, 28, 0x12B7F5, 21, 17, 0x18B9F5, 20, 8, 0x12B7F5, 56, 4, 0x16B8F5, 120, 8, 0x12B7F5, 131, 13, 0x12B7F5, 115, 17, 0x12B7F5, 37, 15, 0x12B7F5, 229, 2, 0x12B7F5, 289, 3, 0x12B7F5, 334, 7, 0xFFFFFF, 340, 19, 0x12B7F5, 277, 30, 0x12B7F5, 240, 20, 0xFFFFFF, 243, 7, 0x9CE1FB, 293, 2, 0xFFFFFF, 316, 5, 0x68D1F9, 336, 14, 0x16B8F5, 545, -3, 0x12B7F5, 595, -3, 0x12B7F5, 593, 11, 0x12B7F5, 557, 16, 0x12B7F5, 538, 16, 0x28BEF6, 580, 1, 0xFFFFFF, 565, 20, 0x12B7F5, 529, 10, 0x12B7F5 }, 90, 23, 64, 629, 107);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
            log_info("---- 加好友发送验证界面2-7.0----");
            return true
        else
...
local function action_application_jiaqun() --执行这个页面的操作--
    if gongneng == "add_group" then  --添加群
        ...
        mSleep(1500)
        page_array["page_sq_send_verification"]:enter()
    else
        cicky(68,82,50);   --点击返回
        mSleep(1500)
        if page_array["page_sq_resuilt"]:quick_check_page() == true then
            return page_array["page_sq_resuilt"]:action()
        end
...
local function check_page_sq_result()
    x, y = findMultiColorInRegionFuzzy({ 0x6ED1F6, -11, 15, 0xBDEAFB, 24, 38, 0x6ED1F6, 65, 28, 0xFFFFFF, 72, 13, 0xFFFFFF, -2, 1, 0x6ED1F6, 2, 16, 0x6ED1F6, 191, 16, 0x6ED1F6, 274, 12, 0xA2E2F9, 339, 14, 0xFFFFFF, 357, 30, 0x6ED1F6, 283, 39, 0x6ED1F6, 233, 26, 0x6ED1F6, 336, 3, 0x6ED1F6, 349, 18, 0xFFFFFF, 275, 27, 0xFFFFFF, 50, 23, 0x6ED1F6, 86, 10, 0x6ED1F6, 61, 5, 0xF0FAFE, 576, 11, 0x6ED1F6, 543, 34, 0x6ED1F6, 525, -1, 0x6ED1F6, 294, 32, 0xFAFEFF, 327, 5, 0x6ED1F6, 281, 4, 0x6ED1F6, 285, 12, 0x6ED1F6 }, 90, 21, 63, 608, 103);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----搜群结果界面2-7.0----");
        return true
    else
        x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, -28, 26, 0xE9F8FE, 71, 40, 0x12B7F5, 76, 15, 0x12B7F5, 16, 15, 0x91DEFA, 16, 16, 0x91DEFA, 224, 30, 0xFFFFFF, 315, 31, 0xFFFFFF, 332, 20, 0xFFFFFF, 282, 4, 0x12B7F5, 247, 9, 0x12B7F5, 316, 35, 0xF1FBFE, 338, 30, 0xC5EDFD, 544, 9, 0x12B7F5, 555, 35, 0x12B7F5, 479, 44, 0x12B7F5, 503, 7, 0x12B7F5, 577, 17, 0x12B7F5, 454, 29, 0x12B7F5, 438, 25, 0x12B7F5, 422, 14, 0x12B7F5, 422, 14, 0x12B7F5 }, 90, 19, 57, 624, 101);
        if x ~= -1 and y ~= -1 then  -- 如果找到了
...
local function action_group_chat() --执行这个页面的操作--
    if gongneng == "group_add_friends" then   --群加好友
        ...
        mSleep(800)
        if text_enter_num_group ~= nil then   --第几个群
            local num_group = tonumber(text_enter_num_group)*110 -110   --进第几个群
            cicky( 150,300+num_group,50);  --点击第几个群
        else
            cicky(150,300,50);          --默认点击第一个群
        end
        mSleep(1000)
...
local function check_page_group_message()
    x, y = findMultiColorInRegionFuzzy({ 0xE8F8FE, 60, -9, 0x12B7F5, 77, -8, 0x3EC4F7, 94, 12, 0x12B7F5, 95, 31, 0x12B7F5, 63, 41, 0x12B7F5, 28, 40, 0x12B7F5, -28, 29, 0x12B7F5, -35, 23, 0xE8F8FE, 2, 12, 0xE8F8FE, 31, 22, 0x12B7F5, 62, 19, 0x12B7F5, 70, 5, 0xCFF0FD, 50, 2, 0x12B7F5, -24, 4, 0x12B7F5, -34, 11, 0xE8F8FE, 17, 38, 0x12B7F5, 61, 34, 0x12B7F5, 74, 20, 0x12B7F5, 57, 7, 0x12B7F5, -3, 1, 0xE8F8FE, -28, 9, 0xD3F2FD }, 90, 495, 59, 625, 109);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
...
local function action_group_message() --执行这个页面的操作--
    ...
    elseif gongneng == "group_add_friends" or gongneng == "pull_people_into_group" then   
        ...
        if page_group_data() == true then
            ...
            --群成员1-9.0
            x2, y2 = findMultiColorInRegionFuzzy({ 0xFEFEFE, 18, 2, 0xFEFEFE, 102, -3, 0x8A8A8A, 142, -3, 0xFEFEFD, 145, 5, 0xFEFEFD, 105, 16, 0xFEFEFE, 68, 18, 0xFEFEFE, 11, 7, 0xFCFCFC, -12, -7, 0xFEFEFE, -1, -5, 0xFBFBFB, 94, 17, 0xFEFEFE, 149, 7, 0xFEFEFD, 98, -14, 0xFEFEFE, 32, -13, 0xFEFEFE, 12, -11, 0xFEFEFE }, 90, 30, 851, 191, 883);
            if x1 ~= -1 or x2 ~= -1 then  -- 如果找到了
                if gongneng == "group_add_friends" then
                    cicky( 278,1025,50);  --点击群成员
                    mSleep(1000)
                    page_array["page_group_member"]:enter()
                else
...
local function check_page_group_member()
    x, y = findMultiColorInRegionFuzzy({ 0x6ED1F6, 6, 36, 0x6ED1F6, 78, 35, 0x6ED1F6, 100, 11, 0x6ED1F6, 51, 6, 0x6ED1F6, 32, 23, 0x6ED1F6, 94, 19, 0x6ED1F6, 253, 21, 0xFFFFFF, 331, 31, 0x6DD0F6, 343, 15, 0x6DD0F6, 276, 2, 0x65C8EE, 272, 13, 0xFFFFFF, 341, 17, 0xFFFFFF, 555, 31, 0x6ED1F6, 601, 22, 0x6ED1F6, 569, 5, 0x6ED1F6, 564, 24, 0x6ED1F6, 582, 24, 0x6ED1F6, 582, 24, 0x6ED1F6 }, 90, 22, 65, 623, 101);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq群成员界面7.0----");
        return true
    else
...
local function action_group_member() --执行这个页面的操作--
    if gongneng == "group_add_friends" then   --群加好友
        ...
        move(  324,230,324,120,4)    --滑动一下
        mSleep(500)
        cicky( 193,248,50);   --点击第一个成员
        mSleep(1500)
        if page_array["page_group_member_data"]:quick_check_page() == true then
            page_array["page_group_member_data"]:action()
        elseif page_array["page_group_member"]:quick_check_page() == true then
            mSleep(1000)
        else
            cicky(31,76,50);   --返回
            mSleep(1000);
...
--复制qq号操作
local function fuzhi_qq_number_action()
   --账号信息1-7.0
  ...
  --账号信息3-7.0
    x3, y3 = findMultiColorInRegionFuzzy({ 0xFFFFFF, 72, 4, 0x818181, 85, 12, 0xFFFFFF, 38, 25, 0xFFFFFF, -21, 15, 0x808080, 35, 15, 0xFFFFFF, 62, 15, 0xFFFFFF, 66, 15, 0x818181 }, 90, 46, 468, 152, 493);
    --账号信息1-9.0
    ...
    if x ~= -1  or x2 ~= -1 or x3 ~= -1 or xx1 ~= -1 then 
        ...
        elseif x3 ~= -1 and y3 ~= -1 then
             cicky( 325,486,1000);  --长按账号
             mSleep(500)
             cicky(294,372,50);    --点击复制
        end
        mSleep(1000)
    ...
    elseif  gongneng == "group_message_add_friends" then
        ...
        for i=1,8 do  
            ...
            elseif page_array["page_group_message_jiaren_send_verification"]:quick_check_page() == true then
                return page_array["page_sgroup_message_jiaren_send_verification"]:action()
            end
            mSleep(1000)
        end
...
local function action_qhy_add_verification() --执行这个页面的操作--
    if gongneng == "group_add_friends" then   --群加好友
        if text_group_add_verification_info ~= nil and text_group_add_verification_info[1] ~= nil then
            cicky( 598,980,2500);   --点击叉叉 删除
            mSleep(500)
            --随机输入验证消息
            inputText(text_group_add_verification_info[math.random(#text_group_add_verification_info)])
            mSleep(1000)
        end
...
local function check_page_space_dynamic()
    x, y = findMultiColorInRegionFuzzy({ 0x12B7F5, 31, 16, 0xFFFFFF, -3, 34, 0x12B7F5, -7, 6, 0x12B7F5, -13, 10, 0x12B7F5, -187, 12, 0x12B7F5, -287, 24, 0xC8EEFD, -312, 11, 0x12B7F5, -299, 0, 0x12B7F5, -200, 5, 0xFFFFFF, -203, 13, 0x22BCF6, -302, 23, 0x12B7F5, -310, 10, 0x12B7F5, -280, 3, 0x75D5F9, -571, 15, 0x12B7F5, -521, 29, 0x12B7F5, -478, 20, 0x12B7F5, -498, 3, 0x12B7F5, -535, 11, 0x12B7F5, -478, 21, 0x66D0F9, -492, 10, 0x12B7F5, -524, 36, 0x12B7F5, -324, 44, 0x12B7F5, -190, 44, 0x12B7F5, -77, 46, 0x12B7F5, 15, 36, 0x12B7F5, 15, 2, 0x12B7F5 }, 90, 7, 68, 609, 114);
    if x ~= -1 and y ~= -1 then  -- 如果找到了
        log_info("----qq好友动态界面----");
        return true
    else
...
local function action_fashuoshuo() --执行这个页面的操作--
    if gongneng == "fashuoshuo" then
        if text_shuoshuo_info ~= nil then
