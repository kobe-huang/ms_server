...
--                                             --
--                                             --
--170609  15:01:38    luoshui package 
--                                             --
--                                             --
--                                             --
-------------------------------------------------
...
function  phone_weixin(file,num_txt)   --读取文本电话号或微信 读一行 删一行
    kong(file);
    shengcheng(file,num_txt) ;     --生成文件
    local t={};
...
function  shengcheng(file,num_txt)
    if false == file_exists(file) then
        for i , v in pairs(num_txt) do
            ...
            local index = get_var_item("var_ms_index")   --获取index
            local num = #num_txt / 10   --把号码分成10分  
            if i > index * (num - 1)  and i <= index * num then  --获取index范围的号码
                local f = io.open(file, 'a');
                f:write(v .. "\r\n");
                f:close();
            end
