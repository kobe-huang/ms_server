...
--                                             --
--                                             --
--170523  10:25:07    luoshui package 
--                                             --
--                                             --
--                                             --
-------------------------------------------------
    ...
    if gongneng == 2.11 then
        ...
        if gong_neng == "新号" then   --加新号为好友
            ...
            elseif add_num > 28 then
                nv_write_nv_item("xinhao_info_add_num",1)   --初始化
            end
...
local function action_liaotianxinxi()     --操作
    ...
    elseif gongneng == "保存" then 
        ...
        x1, y1 = findMultiColorInRegionFuzzy({ 0xFFFFFF, 15, 0, 0xFFFFFF, 46, 1, 0xFFFFFF, 89, 4, 0xFFFFFF, 140, 4, 0xFFFFFF, 166, 12, 0xFFFFFF, 143, 30, 0xFFFFFF, 140, 31, 0xFFFFFF, 75, 31, 0xF7F7F7, 30, 31, 0xFFFFFF, 17, 9, 0xAFAFAF, 108, 14, 0xFFFFFF, 140, 14, 0xFFFFFF, 141, 14, 0xFFFFFF, 46, 8, 0x6C6C6C, 9, 11, 0x787878 }, 90, 42, 249, 208, 280);
         --别人的群  消息免打扰
        x2, y2 = findMultiColorInRegionFuzzy({ 0xFFFFFF, 17, 0, 0x272727, 47, 0, 0xDADADA, 67, 0, 0x101010, 95, 0, 0xECECEC, 145, -3, 0xF5F5F5, 164, -3, 0xFFFFFF, 159, 8, 0xFFFFFF, 143, 14, 0x010101, 89, 17, 0x010101, 46, 17, 0xFFFFFF, 22, 13, 0xFCFCFC, 5, 7, 0x000000, 20, 6, 0x000000, 83, 6, 0xFFFFFF, 122, 9, 0xFFFFFF, 147, 9, 0xFDFDFD, 150, 9, 0x555555 }, 90, 39, 128, 203, 148);
        if x1 ~= -1 or x2 ~= -1 then  -- 如果找到了
            --点击按钮1
            x1, y1 = findMultiColorInRegionFuzzy({ 0xFFFFFF, 25, 2, 0xF0F0F0, 60, 17, 0xFFFFFF, 60, 26, 0xFFFFFF, 8, 36, 0xFFFFFF, 1, 25, 0xFFFFFF, 11, 11, 0xFFFFFF, 37, 11, 0xFBFBFB, 46, 22, 0xFDFDFD, 21, 25, 0xFFFFFF, 15, 11, 0xFFFFFF, 48, 13, 0xFEFEFE }, 90, 535, 245, 595, 281);
            --点击按钮2
            x2, y2 = findMultiColorInRegionFuzzy({ 0xFFFFFF, 24, 0, 0xFFFFFF, 38, 1, 0xE2E2E2, 64, 3, 0xFFFFFF, 73, 9, 0xFFFFFF, 73, 10, 0xFFFFFF, 63, 22, 0xFFFFFF, 35, 30, 0xEFEFEF, 14, 25, 0xFFFFFF, 8, 17, 0xFFFFFF, 38, 9, 0xF7F7F7, 59, 12, 0xFEFEFE, 17, 20, 0xFFFFFF, 3, 9, 0xFFFFFF, 29, 7, 0xFFFFFF, 49, 11, 0xFBFBFB, 61, 16, 0xFFFFFF, 62, 17, 0xFFFFFF }, 90, 528, 132, 601, 162);
            if x1 ~= -1 and y1 ~= -1 then  -- 如果找到了
                cicky(x1,y1,50)
                mSleep(1500)
                warning_info("群消息添加到免打扰")
            elseif x2 ~= -1 and y2 ~= -1 then  -- 如果找到了
                cicky(x2,y2,50)
                mSleep(1500)
                warning_info("群消息添加到免打扰")
            else
                warning_info("群消息已添加到免打扰")
                mSleep(1000)
            end
        end
        --别人的群保存
