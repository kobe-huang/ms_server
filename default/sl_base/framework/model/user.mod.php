...
function members_single($user_or_uid) {//gan
	global $_W;
	...
	if (!empty($user['username'])) {
		...
		$params[':realname'] = $user['username'];

	}


	$sql = 'SELECT * FROM ' . tablename('mc_members') . " $where LIMIT 1";
	$record = pdo_fetch($sql, $params);
	...
	if (!empty($user['password'])) {
		...
		$password = md5($user['password'] . $record['salt'] . $_W['config']['setting']['authkey']);
