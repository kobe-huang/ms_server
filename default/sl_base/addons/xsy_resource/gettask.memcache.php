...
class GETTASK {
    ...
    function task($get){
        ...
        /*kobe 新加20170605*/
        if(!empty($this->m_dev_info['user_id']))
        {
            $data['user_id'] = $this->m_dev_info['user_id'];
        }
        if(!empty($this->m_dev_info['ms_index']))
        {
            $data['ms_index'] = $this->m_dev_info['ms_index'];
        }
    ...
    function check_device(){
        ...
        $this->m_dev_info = cache_read($this->pre_fix_dev_id . 'dev_info');
        $is_change_user = false; //kobe 设备是否更换了用户
        ...
        if(!empty($this->dev_msg['ms_pwd']))
        {
            ...
            {       
                ...
                $ms_index = $this->update_alive($is_change_user);            //更新alive表
                $this->m_dev_info['ms_index'] = $ms_index;
                log_info("22222222");        
    ...
    //刷新alive设备表--
    function update_alive($is_change_user){ 
        $alive_date = array(); 
        ...
        if(empty($ms_id)){//新增
            ...
            if (true == $is_change_user){
            pdo_update("ms_alive_table", 
                    array('ms_type'=>$alive_data['ms_type'],
                        'user_id'=>$alive_data['user_id'], 
                        'account'=>$alive_data['account'],
                        'ms_index' => $index ),               
            ...
            else
            {
                ...
                $ms_index = pdo_fetchcolumn ("SELECT ms_index FROM " .tablename('ms_alive_table') . " WHERE ms_id='".$this->dev_msg['ms_id']."'");
                if (0 ==  $ms_index){
                    $ms_index = $index;
                }
                pdo_update("ms_alive_table", 
    ...
    //查找出可以用的index 给调用 
    function get_dev_new_index($user_id) {
        $index_list = pdo_fetchall ("SELECT ms_index FROM " .tablename('ms_alive_table') . " WHERE user_id='".$user_id."'");
        if (!empty($index_list) )
        {   
            
            $list_size = count($index_list);
            log_info("xxxxx user_id = ".$user_id ."  count=".$list_size );
            for ($index=1; $index < 100000; $index++) { //最多10w个用户,序号从头开始撸
                # code...
                $is_find = false;
                for ($xx=0; $xx < $list_size ; $xx++) { 
                     # code...
                     if( $index_list[$xx]["ms_index"] == $index ){
                        $is_find = true;
                        break;
                     }
                }
