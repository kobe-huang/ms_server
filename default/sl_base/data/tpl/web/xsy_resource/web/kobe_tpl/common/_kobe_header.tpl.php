<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('web/kobe_tpl/common/kobe_header-base', TEMPLATE_INCLUDEPATH)) : (include template('web/kobe_tpl/common/kobe_header-base', TEMPLATE_INCLUDEPATH));?>
	...
	<div class="navbar navbar-inverse navbar-static-top" role="navigation" style="position:static;">
		<div class="container-fluid">
			<ul class="nav navbar-nav">
				...
				<?php  global $top_nav;?>
<?php  error_log(" kobe_header 111") ?>
				<?php  if(is_array($top_nav)) { foreach($top_nav as $nav) { ?>
				<?php  if($nav['title']!=='基础设置' && $nav['title']!=='微站功能') { ?>
					<?php  if(!empty($_W['isfounder']) || empty($_W['setting']['permurls']['sections']) || in_array($nav['name'], $_W['setting']['permurls']['sections'])) { ?>
					<li<?php  if(FRAME == $nav['name']) { ?> class="active"<?php  } ?>>
						<a href="<?php  if($nav['title']=='粉丝营销') { ?><?php  echo url('mc/member/' . $nav['name']);?><?php  } else if($nav['title']=='扩展功能') { ?>./index.php?c=site&a=entry&eid=39<?php  } else { ?><?php  echo url('home/welcome/' . $nav['name']);?><?php  } ?>">
						<i class="<?php  echo $nav['append_title'];?>"></i>
						<?php  if($nav['title']=='粉丝营销') { ?>用户中心<?php  } else if($nav['title']=='扩展功能') { ?>设备管理<?php  } else { ?><?php  echo $nav['title'];?><?php  } ?>
						</a>
					</li>
					<?php  } ?>
				<?php  } ?>
				<?php  } } ?>
<?php  error_log(" kobe_header 222") ?>
<!-- 				<li <?php  if($action == 'emulator') { ?>class="active"<?php  } ?>>
				...
				<?php  if(IMS_FAMILY != 'x') { ?>
<!-- 				<li><a href="http://bbs.yinke.cc"><i class="fa fa-comment"></i>微擎论坛</a></li>
					...
					<div class="panel panel-default">
						...
						<ul class="list-group collapse in" id="frame-<?php  echo $k;?>">
							...
							<?php  if(empty($link['is_permission'])) { ?>
								...
								<?php  } else { ?>
								<a class="list-group-item<?php  echo $link['active'];?>" href="<?php  echo $link['url'];?>" kw="<?php  echo $link['title'];?>"><?php  if($link['title']=='扩展功能管理') { ?>设备功能管理<?php  } else { ?><?php  echo $link['title'];?><?php  } ?></a>
								<?php  } ?>
							<?php  } ?>
							<?php  } } ?>
						</ul>
