...
<!-- 	<title><?php  if(isset($title)) $_W['page']['title'] = $title?>
				<?php  if(!empty($_W['page']['title'])) { ?>
					<?php  echo $_W['page']['title'];?>
				<?php  } ?>
				<?php  if(empty($_W['page']['copyright']['sitename'])) { ?>
				<?php  if(IMS_FAMILY != 'x') { ?> - 自助引擎<?php  } ?>
			<?php  } else { ?>
	...
	<meta name="description" content="<?php  if(empty($_W['page']['copyright']['description'])) { ?><?php  if(IMS_FAMILY != 'x') { ?>管理系统<?php  } ?><?php  } else { ?><?php  echo $_W['page']['copyright']['description'];?><?php  } ?>" />
	...
	window.sysinfo = {
		...
		'account' : <?php  echo "huangyinke"?>//json_encode($_W['account'])}
	};
	</script>
...
<body>
<?php  error_log("end kobe_header-base") ?>