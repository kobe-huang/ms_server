...
function _login($forward = '') {
	...
	if(!empty($record)) {
		...
		if(empty($forward)) {
			...
			// $forward = './index.php?c=system&a=welcome';
		...
		$cookie['hash'] = md5($record['password'] . $record['salt']);
		$session = base64_encode(json_encode($cookie));
		isetcookie('__session', $session, 7 * 86400 , true);
		...
		$forward = './index.php?c=account&a=switch&uniacid=1';//用户登录直接跳转用户界面

		message("欢迎回来，{$record['realname']}。", $forward);
